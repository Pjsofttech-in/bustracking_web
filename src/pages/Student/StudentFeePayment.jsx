...
export default function StudentFeePayment() {
  ...
  const handlePaymentSave = async () => {
    ...
    try {
      ...
      const result = await studentFeePaymentApi.pay(payload);
    ...
    <PageContainer>
      <MainContent>
        <ContentWrapper>
          ...
          {/* Filter Section */}
          <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "stretch", sm: "center" }, gap: { xs: 1.5, sm: 2 }, mb: { xs: 2, sm: 2.5, md: 3 } }}>
            <StyledTextField
              select
              label="Academic Year"
              value={selectedYear}
              ...
              sx={{ minWidth: { xs: "100%", sm: 250, md: 300 }, '& .MuiOutlinedInput-root': { borderRadius: "10px" } }}
              InputProps={{ startAdornment: <InputAdornment position="start"><CalendarTodayIcon sx={{ color: '#94a3b8', fontSize: isExtraSmall ? 16 : 20 }} /></InputAdornment> }}
            >
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", sm: "repeat(4, 1fr)" }, gap: { xs: 1, sm: 1.5, md: 2 }, mb: { xs: 2, sm: 2.5, md: 3 } }}>
            <StatsCard><Typography variant="caption" color="text.secondary">Total Students</Typography><Typography variant="h6" sx={{ fontWeight: 700 }}>{totalStudents}</Typography></StatsCard>
            <StatsCard><Typography variant="caption" color="text.secondary">Paid</Typography><Typography variant="h6" sx={{ fontWeight: 700, color: "#22c55e" }}>{paidStudents}</Typography></StatsCard>
            <StatsCard><Typography variant="caption" color="text.secondary">Pending</Typography><Typography variant="h6" sx={{ fontWeight: 700, color: "#dc2626" }}>{pendingStudents}</Typography></StatsCard>
            <StatsCard><Typography variant="caption" color="text.secondary">Collection Rate</Typography><Typography variant="h6" sx={{ fontWeight: 700, color: "#6495ED" }}>{collectionRate}%</Typography></StatsCard>
          </Box>

          {/* Table/List View */}
          <StyledPaper>
            {loading ? (
              <Box display="flex" justifyContent="center" p={5}><CircularProgress size={isExtraSmall ? 30 : 40} sx={{ color: "#6495ED" }} /></Box>
            ) : (
              isDesktop ? (
                      ...
                      {students.length > 0 ? (
                        students.map((s) => (
                          <StyledTableRow key={s.id}>
                            ...
                            <TableCell><Typography sx={{ fontWeight: 500, fontSize: { xs: '0.6rem', sm: '0.7rem', md: '0.85rem' }, wordBreak: 'break-word' }}>{s.name}</Typography></TableCell>
                            <TableCell align="center">
                              {s.feeStatus === "PAID" ? (
                                <Chip icon={<CheckCircleIcon sx={{ fontSize: { xs: 12, sm: 14, md: 16 } }} />} label="PAID" size="small" sx={{ backgroundColor: "#dcfce7", color: "#16a34a", fontWeight: 600, fontSize: { xs: "0.45rem", sm: "0.55rem", md: "0.75rem" }, borderRadius: "6px", height: { xs: "16px", sm: "18px", md: "24px" }, '& .MuiChip-icon': { color: "#16a34a", fontSize: { xs: 12, sm: 14, md: 16 } } }} />
                              ) : (
                                <Chip icon={<CancelIcon sx={{ fontSize: { xs: 12, sm: 14, md: 16 } }} />} label="PENDING" size="small" sx={{ backgroundColor: "#fee2e2", color: "#dc2626", fontWeight: 600, fontSize: { xs: "0.45rem", sm: "0.55rem", md: "0.75rem" }, borderRadius: "6px", height: { xs: "16px", sm: "18px", md: "24px" }, '& .MuiChip-icon': { color: "#dc2626", fontSize: { xs: 12, sm: 14, md: 16 } } }} />
                              )}
                            </TableCell>
                            <TableCell align="center">
                              {s.feeStatus === "PENDING" && (
                                <PayButton variant="contained" startIcon={<PaymentIcon sx={{ fontSize: { xs: 12, sm: 14, md: 16 } }} />} onClick={() => handlePayClick(s)} disabled={submitting} size={isExtraSmall ? "small" : "medium"}>
                                  Pay Now
                                </PayButton>
                              )}
                              {s.feeStatus === "PAID" && (
                                <Chip label="Completed" size="small" sx={{ backgroundColor: "#dbeafe", color: "#6495ED", fontWeight: 500, fontSize: { xs: "0.45rem", sm: "0.55rem", md: "0.7rem" }, borderRadius: "6px", height: { xs: "16px", sm: "18px", md: "24px" } }} />
                              )}
                            </TableCell>
                          </StyledTableRow>
                        ))
                  ...
                  <Stack spacing={1.5}>
                    {students.length > 0 ? (
                            ...
                            <CardContent sx={{ p: { xs: 1.5, sm: 2, md: 2.5 }, '&:last-child': { pb: { xs: 1.5, sm: 2, md: 2.5 } } }}>
                              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 0.5 }}>
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <Typography variant="h6" sx={{ fontWeight: 600, fontSize: { xs: "0.9rem", sm: "1rem", md: "1.1rem" }, mt: 0.25, wordBreak: 'break-word' }}>{s.name}</Typography>
                                </Box>
                                {s.feeStatus === "PAID" ? (
                                  <Chip icon={<CheckCircleIcon sx={{ fontSize: { xs: 12, sm: 14 } }} />} label="PAID" size="small" sx={{ backgroundColor: "#dcfce7", color: "#16a34a", fontWeight: 600, fontSize: { xs: "0.55rem", sm: "0.6rem", md: "0.65rem" }, borderRadius: "6px", height: { xs: "20px", sm: "22px", md: "24px" }, '& .MuiChip-icon': { color: "#16a34a", fontSize: { xs: 12, sm: 14 } }, flexShrink: 0 }} />
                                ) : (
                                  <Chip icon={<CancelIcon sx={{ fontSize: { xs: 12, sm: 14 } }} />} label="PENDING" size="small" sx={{ backgroundColor: "#fee2e2", color: "#dc2626", fontWeight: 600, fontSize: { xs: "0.55rem", sm: "0.6rem", md: "0.65rem" }, borderRadius: "6px", height: { xs: "20px", sm: "22px", md: "24px" }, '& .MuiChip-icon': { color: "#dc2626", fontSize: { xs: 12, sm: 14 } }, flexShrink: 0 }} />
                                )}
                              </Box>
                              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mt: 1.5, pt: 1.5, borderTop: "1px solid #f1f5f9", flexWrap: "wrap", gap: 0.5 }}>
                                <Box><Typography variant="caption" color="text.secondary">Student ID</Typography><Typography variant="body2" sx={{ fontWeight: 500 }}>#{s.id}</Typography></Box>
                                <Box>
                                  {s.feeStatus === "PENDING" && (
                                    <PayButton variant="contained" startIcon={<PaymentIcon sx={{ fontSize: { xs: 12, sm: 14 } }} />} onClick={() => handlePayClick(s)} disabled={submitting} size={isExtraSmall ? "small" : "medium"}>
                                      Pay Now
                                    </PayButton>
                                  )}
                                  {s.feeStatus === "PAID" && (
                                    <Chip label="Completed" size="small" sx={{ backgroundColor: "#dbeafe", color: "#6495ED", fontWeight: 500, fontSize: { xs: "0.5rem", sm: "0.55rem", md: "0.65rem" }, borderRadius: "6px", height: { xs: "18px", sm: "20px", md: "24px" } }} />
                                  )}
                                </Box>
                              </Box>
                            </CardContent>
                      ...
                      <Box sx={{ textAlign: "center", py: { xs: 3, sm: 4 } }}>
                        ...
                        <Typography variant="body1" color="text.secondary">No students found</Typography>
                        <Typography variant="body2" color="text.secondary">Select an academic year to view students</Typography>
                      </Box>
                    )}
                  </Stack>
                </Box>
          ...
          <StyledDialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
            <DialogTitle sx={{ fontWeight: 700, fontSize: { xs: "0.95rem", sm: "1.1rem", md: "1.25rem" }, color: "#1e293b", display: "flex", justifyContent: "space-between", alignItems: "center", pr: 0.5, p: { xs: 1.5, sm: 2, md: 2.5 } }}>
              Pay Fee
              <IconButton onClick={() => setOpen(false)} size={isExtraSmall ? "small" : "medium"} disabled={submitting}><CloseIcon sx={{ fontSize: { xs: 18, sm: 20, md: 24 } }} /></IconButton>
            </DialogTitle>
            <DialogContent sx={{ p: { xs: 1.5, sm: 2, md: 2.5 } }}>
              {selectedStudent && (
                <Box sx={{ mb: 2, p: { xs: 1.5, sm: 2 }, bgcolor: "#f8fafc", borderRadius: "12px", [theme.breakpoints.down('xs')]: { borderRadius: "8px", p: 1.5 } }}>
                  <Typography variant="caption" color="text.secondary">Student</Typography>
                  <Typography variant="h6" sx={{ fontWeight: 600, fontSize: { xs: "0.9rem", sm: "1rem" }, wordBreak: 'break-word' }}>{selectedStudent.name}</Typography>
                  <Typography variant="caption" color="text.secondary">ID: #{selectedStudent.id}</Typography>
                </Box>
              )}
              <Grid container spacing={isExtraSmall ? 1 : isMobile ? 1.5 : 2}>
                <Grid item xs={12}>
                  <StyledTextField label="Amount" type="number" fullWidth value={payment.amount} onChange={(e) => setPayment({ ...payment, amount: e.target.value })} disabled={submitting} size={isExtraSmall ? "small" : isMobile ? "small" : "medium"} InputProps={{ startAdornment: <InputAdornment position="start"><AttachMoneyIcon sx={{ color: '#94a3b8', fontSize: isExtraSmall ? 16 : 20 }} /></InputAdornment> }} placeholder="Enter amount" />
                </Grid>
                <Grid item xs={12}>
                  <StyledTextField select label="Payment Mode" fullWidth value={payment.paymentMode} onChange={(e) => setPayment({ ...payment, paymentMode: e.target.value })} disabled={submitting} size={isExtraSmall ? "small" : isMobile ? "small" : "medium"}>
