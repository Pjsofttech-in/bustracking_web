...
export default function Bustrip() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // ✅ Using direct API imports instead of `api` object
      const [tripsData, busesData, routesData, driversData, conductorsData] = await Promise.all([
        busTripApi.getAll(),          // ✅ Direct import usage
        busApi.getAll(),              // ✅ Direct import usage
        busRouteApi.getAll(),         // ✅ Direct import usage
        driverApi.getAll().catch(() => []),      // ✅ Direct import usage
        conductorApi.getAll().catch(() => [])    // ✅ Direct import usage
      ]);
          ...
          <Box sx={{ 
            ...
            <StatsCard>
              ...
              <Typography variant="h6" sx={{ fontWeight: 700, fontSize: { xs: "0.85rem", sm: "1rem", md: "1.25rem" }, color: "#22c55e" }}>
                {trips.filter(t => t.tripStatus === 'ONGOING').length}
              </Typography>
            </StatsCard>
            <StatsCard>
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: { xs: "0.5rem", sm: "0.6rem", md: "0.7rem" } }}>
              ...
              <Typography variant="h6" sx={{ fontWeight: 700, fontSize: { xs: "0.85rem", sm: "1rem", md: "1.25rem" }, color: "#6495ED" }}>
                {trips.filter(t => t.tripStatus === 'SCHEDULED').length}
              </Typography>
            </StatsCard>
            <StatsCard>
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: { xs: "0.5rem", sm: "0.6rem", md: "0.7rem" } }}>
                        ...
                        <StyledTableRow key={trip.id}>
                          ...
                          <TableCell>
                            <Chip 
                              ...
                              size="small"
                              sx={{
                                backgroundColor: getStatusColor(trip.tripStatus).bg,
                                color: getStatusColor(trip.tripStatus).color,
                                fontWeight: 600,
          ...
          <Grid container spacing={isExtraSmall ? 1 : isMobile ? 1.5 : 2} sx={{ mt: 0 }}>
            ...
            <Grid item xs={12} sm={6}>
              <StyledAutocomplete
                ...
                onChange={(e, v) =>
                  setFormData({ ...formData, busId: v?.id || "" })
                }
                disabled={submitting}
                renderInput={(params) => (
                  <StyledTextField 
            ...
            <Grid item xs={12} sm={6}>
              <StyledAutocomplete
                ...
                onChange={(e, v) =>
                  setFormData({ ...formData, routeId: v?.id || "" })
                }
                disabled={submitting}
                renderInput={(params) => (
                  <StyledTextField 
            ...
            <Grid item xs={12} sm={6}>
              <StyledAutocomplete
                ...
                onChange={(e, v) =>
                  setFormData({ ...formData, driverId: v?.id || "" })
                }
                disabled={submitting}
                renderInput={(params) => (
                  <StyledTextField 
            ...
            <Grid item xs={12} sm={6}>
              <StyledAutocomplete
                ...
                onChange={(e, v) =>
                  setFormData({ ...formData, conductorId: v?.id || "" })
                }
                disabled={submitting}
                renderInput={(params) => (
                  <StyledTextField 
            ...
            <Grid item xs={12} sm={6}>
              <StyledTextField
                ...
                onChange={(e) =>
                  setFormData({ ...formData, tripStatus: e.target.value })
                }
                disabled={submitting}
                size={isExtraSmall ? "small" : isMobile ? "small" : "medium"}
              >
                <MenuItem value="" sx={{ fontSize: { xs: '0.75rem', sm: '0.875rem' } }}>Select Status</MenuItem>
                {statusOptions.map((status) => (
                  <MenuItem key={status} value={status} sx={{ fontSize: { xs: '0.75rem', sm: '0.875rem' } }}>
                    {status}
                  </MenuItem>
                ))}
              </StyledTextField>
